...
@router.get("/{user_id}/all", response_model=List[Task])
async def get_all_tasks(user_id: str, db=Depends(get_db)) -> List[Task]:
...
async def get_tasks(user_id: str, task_id: str, db=Depends(get_db)):
    task_crud = TaskCRUD(db)
...
@router.post("/{user_id}")
async def create_task(user_id: str, task: CreateTask, db=Depends(get_db)):
    task_crud = TaskCRUD(db)
...
@router.put("/{user_id}")
async def update_task(user_id: str, task: UpdateTask, db=Depends(get_db)):
    task_crud = TaskCRUD(db)
